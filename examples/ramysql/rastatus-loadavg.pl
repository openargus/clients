...
#  QoSient, LLC
#
#  rastatus-loadavg - Report a number of system status variables.
#
#
...
if (open(SESAME, $file)) {
   while ($data = <SESAME>) {
      if (length($data) > 0) {
         ...
         $data_results->{'1minAvg'}  = $fields[0];
         $data_results->{'5minAvg'}  = $fields[1];
         $data_results->{'15minAvg'} = $fields[2];
         $data_results->{'procs'}    = $fields[3];
...
if ($json == 0) {
   foreach my $entry (@{ $results_ref }) {
      print "     <Data Date=\"$entry->{'date'}\" 1minAvg=\"$entry->{'1minAvg'}\" 5minAvg=\"$entry->{'5minAvg'}\" 15minAvg=\"$entry->{'15minAvg'}\" Procs=\"$entry->{'procs'}\" \\>\n"
   }
   ...
   print JSON->new->utf8->space_after->encode({loadavg => $results_ref});
   print "\n";
}

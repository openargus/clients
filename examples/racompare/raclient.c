...
/*
 * $Id: //depot/gargoyle/clients/examples/racompare/raclient.c#40 $
...
ArgusProcessData (void *arg)
{
   ...
   while (!ArgusCloseDown && !done) {
      if (parser->RaTasksToDo) {
         ...
         if ((!(parser->status & ARGUS_BASELINE_LIST_PROCESSED)) && ((file = (struct ArgusFileInput *)parser->ArgusBaselineList) != NULL))
            ArgusLoadBaselineFiles(parser);

         if ((!(parser->status & ARGUS_FILE_LIST_PROCESSED)) && ((file = (struct ArgusFileInput *)parser->ArgusInputFileList) != NULL)) {
            ...
            while (file && parser->eNflag) {
               switch (file->type) {
                  ...
                  case ARGUS_FLOW_TOOLS_SOURCE: {
                     parser->ArgusCurrentInput = (struct ArgusInput *) file;
...
*/
         ...
         if ((parser->ArgusBaselineList != NULL) || (parser->ArgusInputFileList != NULL)) {
            parser->RaTasksToDo = RA_ACTIVE;
...
RaProcessThisRecord (struct ArgusParserStruct *parser, struct ArgusRecordStruct *ns)
{
   ...
   if (ArgusProcessingBaseline) {
      ...
      baselinefound = ArgusFindRecordInBaseline(parser, ns);
            ...
            while ((tns = ArgusAlignRecord(parser, cns, &RaBinProcess->nadp)) != NULL) {
               ...
               if (pns) {
                  if (pns->bins) {
                     ...
                     if (ArgusInsertRecord (parser, pns->bins, tns, offset) <= 0) {
...
RaParseLsOfEventRecord (struct ArgusParserStruct *parser, char *dptr, struct ArgusRecordStruct *argus, int status)
{
   ...
   ArgusPrintSourceID(parser, sptr, argus, 48);
...
//                   struct ArgusRecordStruct *cns = ArgusCopyRecordStruct(ns);
                     ...
                     ArgusAddToQueue (queue, &ns->qhdr, ARGUS_NOLOCK);
                     ns->status |= ARGUS_RECORD_BASELINE | ARGUS_NSR_STICKY;
...
ArgusCreateSQLSaveTable(char *table)
{
   ...
   char stable[256], sbuf[MAXSTRLEN], kbuf[MAXSTRLEN - 64];
...
ArgusLoadBaselineFiles (struct ArgusParserStruct *parser)
{
   ...
   if ((file = (struct ArgusFileInput *)parser->ArgusBaselineList) != NULL) {
      ArgusProcessingSample = 0;

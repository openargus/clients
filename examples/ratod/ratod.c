...
ArgusTimeAdjustRecord(struct ArgusAdjustStruct *nadp, struct ArgusRecordStruct *tns)
{
   ...
   if (time != NULL) {
      struct tm tmbuf, *tmsp = &tmbuf;
 
      tsec =  ArgusFetchStartTime(tns);
      localtime_r(&tsec, tmsp);
   ...
   if (nsec > 0) {
      ...
      if (time->src.start.tv_sec) { time->src.start.tv_sec -= nsec; time->src.start.tv_sec %= size; }
      if (time->dst.start.tv_sec) { time->dst.start.tv_sec -= nsec; time->dst.start.tv_sec %= size; }
      if (time->src.end.tv_sec)   { time->src.end.tv_sec   -= nsec; time->src.end.tv_sec %= size; }
      if (time->dst.end.tv_sec)   { time->dst.end.tv_sec   -= nsec; time->dst.end.tv_sec %= size; }
   }
}

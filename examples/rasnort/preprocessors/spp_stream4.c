/* $Id: spp_stream4.c,v 1.156.2.2 2004/01/27 17:21:23 jh8 Exp $ */
...
void SegmentCleanTraverse(Stream *s)
{
    ...
    while(spd != NULL)
    {
        if(spd->chuck == 1 || SEQ_GEQ(s->last_ack,(spd->seq_num+spd->payload_size)))
        {
...
void Stream4InitReassembler(u_char *args)
{
    ...
    while(i < num_toks)
    {
        ...
        else if(!strncasecmp(index, "flush_data_diff_size", strlen("flush_data_diff_size")))
        {
 ...
 *      if the sequence is > the next expected sequence but still within the window , queue it, and ignore it for now.
 *      if the last ack we received is less than our next sequence, we have outstanding acks - ignore the reset.
 *      
 * ignoring a reset does the following:
 * 	the session is not closed.
 * 	if the session is closed by the receiver of the reset, the session will time out.
 * 	if the session is not closed by the receiver, than data will continue to be tracked.
 * 
 * Includes Fix for bug 2161  
 * 9/2/2003
 *
 * 'go to the river called state, eat any of it's acks - but fear the reset, for it can be poisonous' - man
 * 
 * 
 */
int CheckRst(Session *ssn, int direction, u_int32_t pkt_seq, Packet *p)
{
    ...
    DEBUG_WRAP(foo.s_addr=s->ip; DebugMessage(DEBUG_STREAM, "--RST packet from %s!\n",inet_ntoa(foo));
       DebugMessage(DEBUG_STREAM, "--pkt seq: %u   last_ack: %u base-seq: %u next-seq: %u bytes-sent: %u bytes-tracked: %u"
              "win: %u \n",
	       pkt_seq,s->last_ack,s->base_seq,s->next_seq,s->bytes_sent,s->bytes_tracked,s->win_size););
    }
    /*
     *  We want to make sure the RST has the next valid sequence that this side should be sending
     *  If the pkt_seq < next_seq it's essentially a duplicate sequence, and is probably going to be discarded,
     *  it certainly should be. Also, the base sequence includes the SYN sequence count.
     *  If the packet seq is after the next seq than we should queue the packet for later, in case an
     *  out of order packet arrives. We should also honor the RST-ACK requirements..but I have to research that more.
     *
     *  Ignoring a RST implies we won't shutdown this session due to it.
     *  
     *  This is a standard TCP/IP stack 'in the window' check, but it's not always the way
     *  stacks handle RST's:
     *  
     *  if(SEQ_LT(pkt_seq,s->base_seq+s->bytes_sent) || SEQ_GEQ(pkt_seq,(s->last_ack+s->win_size))) 
     *  
     *  We use a tighter constraint...
     */
    if( !SEQ_EQ(pkt_seq,s->base_seq+s->bytes_sent) )
    {
        DEBUG_WRAP(DebugMessage(DEBUG_STREAM, "Bad RST packet, bad sequence or no ack, no cookie!\n");
	...
	DEBUG_WRAP(DebugMessage(DEBUG_STREAM,"Ignoring a RST (1)...pkt_seq=%u\n",pkt_seq););
        return 0;
    }

    /* At this point if the reset seq + ack flags are ok, we still must not have any data
     * waiting for an ack to honor the reset right now...
     *
     * 9/2/2003 -  bug 2161
     * 
     ...
     */
    if( ubi_sptFind(&s->data,(ubi_btItemPtr)(&spd)) && SEQ_LT(s->last_ack,s->base_seq+s->bytes_sent) )
    {
	DEBUG_WRAP(DebugMessage(DEBUG_STREAM,"Ignoring a RST (2)...pkt_seq=%u\n",pkt_seq););
        return 0;
    }

    DEBUG_WRAP(DebugMessage(DEBUG_STREAM,"Not Ignoring a RST...pkt_seq=%u\n",pkt_seq););

    return 1;
}


int CheckRst_orig(Session *ssn, int direction, u_int32_t pkt_seq, Packet *p)
{
    Stream *s;
    Event event;

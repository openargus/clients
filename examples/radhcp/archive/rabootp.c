...
/*
 * $Id: //depot/gargoyle/clients/examples/radhcp/rabootp.c#3 $
 * $DateTime: 2016/06/08 00:19:40 $
 * $Change: 3165 $
 */

/*
 *     rabootp.c  - parse DHCP transactions from argus data
 *                  extract DHCP query and response information into a DHCP structure
...
static const u_char *ns_nparse(struct ArgusDhcpQueryStruct *, register const u_char *, register const u_char *, int);
static const u_char *ns_rparse(struct ArgusDhcpQueryStruct *, register u_char *, register const u_char *, int, int);
...
struct ArgusDhcpQueryStruct *ArgusParseDHCPBuffer (struct ArgusParserStruct *, struct ArgusDataStruct *);
...
ArgusParseDHCPBuffer (struct ArgusParserStruct *parser, struct ArgusDataStruct *user)
{
   struct ArgusDhcpQueryStruct *query = NULL;
   ...
   if (user != NULL) {
      ...
      if ((query = ArgusCalloc(1, sizeof(struct ArgusDhcpQueryStruct))) == NULL)
...
ArgusParseDHCPRecord (struct ArgusParserStruct *parser, struct ArgusRecordStruct *argus, struct ArgusDhcpStruct *dns)
{
   ...
   if (argus != NULL) {
      ...
      if (suser != NULL) {
         if ((query = ArgusParseDHCPBuffer (parser, suser)) != NULL)
            dns->request = query;
      }

      if (duser != NULL) {
         if ((query = ArgusParseDHCPBuffer (parser, duser)) != NULL)
            dns->response = query;
      }
...
static const u_char *
ns_nparse(struct ArgusDhcpQueryStruct *query, register const u_char *cp, register const u_char *bp, int state)
{
   register u_int i, l;
...
static const u_char *
ns_rparse(struct ArgusDhcpQueryStruct *query, register u_char *bp, register const u_char *cp, int state, int is_mdns)
{
   register const u_char *rp = NULL;
